...
// 循环读取客户数据到m_read_buffer中，并更新m_read_idx，直到无数据可读或对方关闭连接
 ...
 * 
 * 页面跳转机制: 浏览器网址栏中的字符，即url，可以将其抽象成ip:port/xxx，xxx通过html文件的action属性进行设置。
...
/**
    ...
    并调用http_conn::write函数将响应报文发送给浏览器端。该函数具体逻辑如下：
    ...
    根据返回值更新byte_have_send和iovec结构体的指针和长度，并判断响应报文整体是否发送成功。
        ...
        2、若writev单次发送不成功，判断是否是写缓冲区满了。
            ...
            * 若eagain则满了，更新iovec结构体的指针和长度，并注册写事件，等待下一次写事件触发
...
bool http_conn::write() {
    ...
    while (1) {
        ...
        // #include <sys/uio.h>
        // ssize_t writev(int filedes /*文件描述符*/ , 
        //                const struct iovec *iov /*io向量机制结构体iovec*/, 
        //                int iovcnt /*iov个数*/);
 ...
 *    add_blank_line添加空行
 *   上述函数均是内部调用add_response函数更新m_write_idx指针和缓冲区m_write_buf中的内容。
 */
 ...
 *
 * iovec是一个结构体，里面有两个元素:
 * 1、指针成员iov_base指向一个缓冲区，这个缓冲区是存放的是writev将要发送的数据。 
